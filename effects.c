...
void split (SDL_Surface* s, SDL_Rect* r_in, Uint32 ticks, split_t type){
  ...
  black=SDL_CreateRGBSurface(SDL_HWSURFACE, r->w, r->h, s->format->BitsPerPixel, s->format->Rmask, s->format->Gmask, s->format->Bmask, s->format->Amask);
  copy= SDL_CreateRGBSurface(SDL_HWSURFACE, r->w, r->h, s->format->BitsPerPixel, s->format->Rmask, s->format->Gmask, s->format->Bmask, s->format->Amask);
